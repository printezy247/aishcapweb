...
export default function GoldChart({ points, intraday }: { points: SeriesPoint[]; intraday: boolean }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(el, {
      ...
      grid: { vertLines: { color: "rgba(168,164,156,0.06)" }, horzLines: { color: "rgba(168,164,156,0.08)" } },
      rightPriceScale: { borderVisible: false, scaleMargins: { top: 0.12, bottom: 0.08 } },
      timeScale: { borderVisible: false, timeVisible: intraday, secondsVisible: false, fixLeftEdge: true, fixRightEdge: true },
      crosshair: { horzLine: { color: "rgba(212,160,23,0.5)", labelBackgroundColor: "#1E1E22" }, vertLine: { color: "rgba(212,160,23,0.5)", labelBackgroundColor: "#1E1E22" } },
      handleScroll: false,
      handleScale: false,
